...
list_menu = {
	...
	"people": [
		"Mass Setujui Permintaan Teman" + DAN,
		"Mass Hapus Permintaan Teman" + DAN,
...
def home():
   ...
   {C}0).{W} Keluar""")
	...
	if pilih == 0:
		banner()
		print("    Terimakasih Telah Menggunakan Tools Ini ^_^")
	elif pilih == 1:
		if not check_login():
...
def login():
   ...
   5. By using this the user is considered to
	...
	if ses.logged:
		...
		print(f"{INF}Login Sukses Boss!")
		enter()
	else:
		print(ERR + "Cookie Tidak Valid!")
		enter()
...
def enter():
	...
	if TOTAL_ENTER > 8:
		exit()
	getpass(f"\n   {C}[{W} Enter Untuk Melanjutkan {C}]{W}")
	CURRENT_FUNC()
	exit()
